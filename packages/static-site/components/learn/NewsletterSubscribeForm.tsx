...
import { getApiBaseUrl } from "@/domain/api/apiBaseUrl";
...
export interface NewsletterSubscribeMessages {
  ...
  readonly subscribeSubmitting: string;
...
const NewsletterSubscribeForm = ({ messages }: Props) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      const response = await fetch(`${getApiBaseUrl()}/api/external/newsletter/subscribe`, {
        method: "POST",
  ...
  return (
    <section className="funding-subscribe-col border-1px border-base-lighter padding-3 radius-lg margin-y-3">
      <h2
        ...
        className="margin-top-0 margin-bottom-1 font-sans-md text-bold"
      >
      ...
      <form onSubmit={handleSubmit} aria-labelledby="newsletter-subscribe-heading">
        <div className={invalidEmail ? "usa-form-group--error" : undefined}>
          ...
          <div className="display-flex flex-align-start">
            ...
            >
              {isSubmitting ? messages.subscribeSubmitting : messages.subscribeButton}
            </button>
          </div>
          {invalidEmail && (
