...
import {
  appTypeKeysString,
  licenseAppType,
  RgbDynamicsLicenseApplicationIdsClient,
} from "@client/dynamics/license-status/RgbDynamicsLicenseApplicationIdsClient";
import {
  BusinessIdAndName,
...
import {
  HEALTH_CLUB_LICENSE_UUID,
  PUBLIC_MOVERS_LICENSE_UUID,
  RGB_APP_TYPE_KEYS,
...
describe("RgbDynamicsLicenseApplicationIdClient", () => {
  ...
  beforeEach(() => {
    ...
    client = RgbDynamicsLicenseApplicationIdsClient(logger, ORG_URL);
  });
  ...
  describe("licenseAppType", () => {
    ...
    for (const application of RGB_LICENSE_APPLICATION_INFORMATION) {
      it(`returns object whose professionNameAndLicenseType key is ${application.expectedProfession}`, async () => {
        const mockLicenseApplicationIdResponse = {
          value: [
            generateLicenseApplicationIdApiResponseValue({
              rgb_number: "39PM00105300",
              ...
              statecode: 0,
              statuscode: 100000006,
              [RGB_APP_TYPE_KEYS[application.uuid]]: application.appTypeCode,
              _rgb_apptypeid_value: application.uuid,
            }),
          ],
        };
        ...
        expect(repsonse[0].professionNameAndLicenseType).toEqual(application.expectedProfession);
      });
    }
  });
});
