...
import { XrayRegistrationSearch } from "@shared/xray";
...
describe("xrayRegistrationSearchClient", () => {
  ...
  it("makes a call to the business name endpoint and returns an array of xray registration entries", async () => {
    ...
    expect(mockAxios.get).toHaveBeenCalledWith(`${ORG_URL}/xray_by_business_name?namepart=${businessName}`);
  ...
  it("makes a call to the address endpoint and returns an array of xray registration entries", async () => {
    ...
    expect(mockAxios.get).toHaveBeenCalledWith(
      `${ORG_URL}/xray_by_address?partialaddr=${addressLine1}&zip=${addressZipCode}`
    );
  ...
  it("throws a NO_ENTRIES_FOUND error when no entries are found from the business name endpoint", async () => {
    mockAxios.get.mockResolvedValue({ data: { data: [] } });
    await expect(client.searchByBusinessName(businessName)).rejects.toThrow("NO_ENTRIES_FOUND");
  });

  it("throws a NO_ENTRIES_FOUND error when no entries are found from the address endpoint", async () => {
    mockAxios.get.mockResolvedValue({ data: { data: [] } });
    await expect(client.searchByAddress(addressLine1, addressZipCode)).rejects.toThrow("NO_ENTRIES_FOUND");
  });
});
