...
describe("AWSCryptoFactory", () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks();
    client = AWSCryptoFactory("some-key", {
      stage: "some-stage",
  ...
  describe("hashValue", () => {
    it("should generate a hash for a valid sensitive string", async () => {
      const hash = await client.hashValue("123-45-6789", "test-application-salt", 1000);
    ...
    it("should generate the same hash for the same input (deterministic)", async () => {
      const hash1 = await client.hashValue("123-45-6789", "test-application-salt", 1000);
      const hash2 = await client.hashValue("123-45-6789", "test-application-salt", 1000);
      expect(hash1).toEqual(hash2);
    });

    it("should generate different hashes for different sensitive strings", async () => {
      const hash1 = await client.hashValue("123-45-6789", "test-application-salt", 1000);
      const hash2 = await client.hashValue("987-65-4321", "test-application-salt", 1000);
      expect(hash1).not.toEqual(hash2);
    });

    it("should normalize inputs by removing non-alphanumeric characters", async () => {
      const hash1 = await client.hashValue("123-45-6789", "test-application-salt", 1000);
      const hash2 = await client.hashValue("123456789", "test-application-salt", 1000);
      expect(hash1).toEqual(hash2);
    });

    it("should throw an error for empty or non-string input", async () => {
      await expect(client.hashValue("", "test-application-salt", 1000)).rejects.toThrow(
        "Sensitive data must be a non-empty string",
      );
      await expect(
        client.hashValue(null as unknown as string, "test-application-salt", 1000),
      ).rejects.toThrow("Sensitive data must be a non-empty string");
      await expect(
        client.hashValue(undefined as unknown as string, "test-application-salt", 1000),
      ).rejects.toThrow("Sensitive data must be a non-empty string");
      await expect(
        client.hashValue(123 as unknown as string, "test-application-salt", 1000),
      ).rejects.toThrow("Sensitive data must be a non-empty string");
    });
    ...
    it("should throw with proper error message when pbkdf2 throws an Error", async () => {
      ...
      jest.spyOn(cryptoUtils, "pbkdf2").mockImplementationOnce(
      ...
      await expect(client.hashValue("123-45-6789", "test-application-salt", 1000)).rejects.toThrow(
        "Hash generation failed: Specific error message",
      );
    });

    it("should iterate 100000 times by default", async () => {
      // Changing the iteration default from 100000 could disrupt production data
      // This test ensures that the default iteration count is 100000
      // and that the hash function is called with the correct parameters
      jest.spyOn(cryptoUtils, "pbkdf2").mockImplementationOnce(
        (_password, _salt, iterations, _keylen, _digest, callback) => {
          callback(null, Buffer.alloc(64));
          return;
        },
      );

      await client.hashValue("123-45-6789", "test-application-salt");
