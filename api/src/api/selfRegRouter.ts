  ...
  router.post("/self-reg", async (req, res) => {
    ...
    try {
      ...
      messagingServiceClient
        .sendEmail(cleanedUserData.user.email, {
          templateId: "welcome-email",
          templateData: { name: cleanedUserData.user.name || "" },
        })
        .then((result) => {
          if (result.success) {
            logger.LogInfo(
              `Welcome email sent successfully to ${cleanedUserData.user.email}, messageId: ${result.messageId}`,
            );
          } else {
            logger.LogError(
              `Failed to send welcome email to ${cleanedUserData.user.email}: ${result.error}`,
            );
          }
        })
        .catch((error) => {
          logger.LogError(
            `Error sending welcome email to ${cleanedUserData.user.email}: ${error instanceof Error ? error.message : String(error)}`,
          );
        });
