...
export const emergencyTripPermitRouterFactory = (
  ...
  router.post(
    ...
    async (req: ExpressRequestBody<EmergencyTripPermitApplicationInfo>, res) => {
      ...
      logger.LogInfo(`[START] ${method} ${endpoint} - submitting emergency trip permit request`);
      ...
      emergencyTripPermitClient
        ...
        .then(async (response: EmergencyTripPermitSubmitResponse) => {
          if ("Success" in response && response.Success) {
            ...
            logger.LogError(
              `${method} ${endpoint} - status: ${status}, failed to submit emergency trip permit due to bad request, duration: ${getDurationMs(
                requestStart,
              )}ms`,
            );
            res.status(status).json(response);
