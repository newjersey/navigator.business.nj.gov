  ...
  const migrateOutdatedVersionUsers = async (): Promise<{
    ...
    try {
      ...
      do {
        ...
        const migratedBatchCounts = await Promise.all(
          batches.map(async (batch) =>
            limit(async () => {
              await processBatch(batch);
              logger.LogInfo(`Processed batch of ${batch.length} users.`);
              return batch.length;
            })
          )
        );

        migratedCount += migratedBatchCounts.reduce((sum, count) => sum + count, 0);
        logger.LogInfo(`Total migrated so far: ${migratedCount}`);
