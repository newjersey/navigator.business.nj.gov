...
export const migrate_v193_to_v194 = (userData: v193UserData): v194UserData => {
  return {
    ...userData,
    user: migrate_v193BusinessUser_to_v194BusinessUser(userData.user),
    businesses: Object.fromEntries(
      Object.values(userData.businesses)
        .map((business: v193Business) => migrate_v193Business_to_v194Business(business))
        .map((currBusiness: v194Business) => [currBusiness.id, currBusiness]),
    ),
    version: 194,
  };
};

const migrate_v193BusinessUser_to_v194BusinessUser = (user: v193BusinessUser): v194BusinessUser => {
  const migratedUser: v194BusinessUser & { abExperience?: unknown } = { ...user };
  delete migratedUser.abExperience;
  return migratedUser;
};

const migrate_v193Business_to_v194Business = (business: v193Business): v194Business => {
  // The select-industry task only ever existed in the removed A/B experience, so any
  // stored progress for it is now orphaned.
  const taskProgress = { ...business.taskProgress };
  delete taskProgress["select-industry"];
