...
export const migrate_v181_to_v182 = (userData: v181UserData): v182UserData => {
  return {
    ...userData,
    user: migrate_v181BusinessUser_to_v182BusinessUser(userData.user),
...
const migrate_v181BusinessUser_to_v182BusinessUser = (user: v181BusinessUser): v182BusinessUser => {
  return {
    ...user,
...
export interface v182ProfileDocuments {
  formationDoc: string;
  standingDoc: string;
  certifiedDoc: string;
}
...
export type v182LicenseDetails = {
...
export type v182LicenseData = {
  lastUpdatedISO: string;
  licenses?: v182Licenses;
};

export type v182Preferences = {
...
export type v182LicenseStatusItem = {
  title: string;
...
export type v182SectionType = (typeof v182SectionNames)[number];

export type v182ExternalStatus = {
  newsletter?: v182NewsletterResponse;
  userTesting?: v182UserTestingResponse;
};

export interface v182NewsletterResponse {
  success?: boolean;
  status: v182NewsletterStatus;
}

export interface v182UserTestingResponse {
  success?: boolean;
...
export interface v182NameAvailabilityResponse {
  status: v182NameAvailabilityStatus;
  similarNames: string[];
  invalidWord?: string;
}

export interface v182NameAvailability extends v182NameAvailabilityResponse {
...
export type v182ForeignGoodStandingFileObject = {
  Extension: "PDF" | "PNG";
  Content: string;
};

export type v182StateObject = {
  shortCode: string;
  name: string;
};

export interface v182FormationAddress {
...
export interface v182FormationSigner {
  readonly name: string;
  readonly signature: boolean;
  readonly title: v182SignerTitle;
}

export interface v182FormationIncorporator extends v182FormationSigner, v182FormationAddress {}

export interface v182FormationMember extends v182FormationAddress {
  readonly name: string;
}
...
export type v182FormationSubmitResponse = {
  success: boolean;
...
export type v182FormationSubmitError = {
  field: string;
  type: "FIELD" | "UNKNOWN" | "RESPONSE";
  message: string;
};

export type v182GetFilingResponse = {
  success: boolean;
  entityId: string;
