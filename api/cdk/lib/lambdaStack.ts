...
import { CONTENT_STAGE, SERVICE_NAME, TESTING_STAGE, PROD_STAGE, DEV_STAGE } from "./constants";
...
import { createLambdaFunction } from "./stackUtils";
...
export class LambdaStack extends Stack {
  ...
  public readonly githubOauth2!: IFunction;
  public readonly subnets: ISubnet[] = [];
  public readonly securityGroups: ISecurityGroup[] = [];
  public readonly lambdas: { [key: string]: ReturnType<typeof createLambdaFunction> } = {};
  ...
  constructor(scope: Construct, id: string, props: LambdaStackProps) {
    ...
    const usersTable = `users-table-${props.stage}`;
    const documentS3Bucket = `nj-bfs-user-documents-${props.stage}`;
    const businessesTable = `businesses-table-${props.stage}`;
    ...
    if (props.stage !== "local") {
      const vpcId = ssm.StringParameter.valueForStringParameter(this, `/${props.stage}/vpc/id`);
      const subnetId1 = ssm.StringParameter.valueForStringParameter(
        this,
        `/${props.stage}/subnet/01`,
      );
      const subnetId2 = ssm.StringParameter.valueForStringParameter(
        this,
        `/${props.stage}/subnet/02`,
      );
      const sgId = ssm.StringParameter.valueForStringParameter(
        this,
        `/${props.stage}/security-group`,
      );
    ...
    // this.vpc = Vpc.fromLookup(this, "ImportedVpc", { vpcId: vpcId });
    //
    // this.subnets.push(Subnet.fromSubnetId(this, "Subnet01", subnet_id_1), Subnet.fromSubnetId(this, "Subnet02", subnet_id_2));
    //
    // this.securityGroups.push(SecurityGroup.fromSecurityGroupId(this, "ImportedSG", security_group_id));
