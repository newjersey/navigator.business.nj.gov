 ...
 *
 * These are shared review environments reached through external WAF routing, while production
 * stays public and local development is intentionally unaffected.
 */
...
interface StaticSiteBasicAuthEnvironment {
  /** Username injected into the ECS task for non-production Basic Auth. */
  readonly username: string;

  /** Password injected into the ECS task for non-production Basic Auth. */
...
/**
 * Reads a GitHub Actions-provided environment variable required for non-production Basic Auth.
 */
...
/**
 * Creates the ECS container environment while only enabling Basic Auth outside production.
 */
const createStaticSiteContainerEnvironment = (
  ...
  if (props.stage === PROD_STAGE || !isStaticSiteBasicAuthStage(props.stage)) {
    return {
      ...baseEnvironment,
