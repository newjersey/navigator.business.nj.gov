...
export class DataStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props: DataStackProps) {
    ...
    const usersTableName = `users-table-${props.stage}`;
    const businessesTableName = `businesses-table-${props.stage}`;
    ...
    if (!process.env.CI || props.stage === "local") {
      ...
      businessesTable.addGlobalSecondaryIndex({
        ...
        partitionKey: { name: "encryptedTaxId", type: dynamodb.AttributeType.STRING },
        projectionType: dynamodb.ProjectionType.ALL,
      });
