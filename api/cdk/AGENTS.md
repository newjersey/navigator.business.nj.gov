...
- `lib/storageStack.ts`, `lib/backupStack.ts`, `lib/encryptionStack.ts`, and
  `lib/monitoringStack.ts` handle S3 storage, backup, KMS, and CloudWatch.
- `lib/staticSiteRepositoryStack.ts` owns the shared ECR repository for
  static-site images, including the tagged-image retention policy.
- `lib/staticSiteClusterStack.ts` owns the shared ECS cluster used by
  static-site services.
- `lib/staticSiteServiceStack.ts` defines the static-site Fargate service,
  ALB, ACM, log group, and SNS wiring.
- `lib/staticSiteTaskDefinition.ts` builds the static-site ECS task
  definition and container environment.
- `lib/staticSiteMonitoring.ts` defines CloudWatch alarms and deployment
  failure event rules for the static site.
- `lib/staticSiteOutputs.ts` declares CloudFormation outputs for the
  static-site service.
