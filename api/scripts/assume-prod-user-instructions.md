...
4. The last thing we need to do is set a few environment variables to use in the script. In `api/.env`, set the following with the proper values:

   ```properties
   AWS_PROFILE_PROD=profile-name-from-step-2
   AWS_PROFILE_DEV=profile-name-from-step-3
   DYNAMODB_TABLE_DEV=secure-this-credential
   DYNAMODB_TABLE_PROD=secure-this-credential
   ```

5. Now that we have profiles for both environments and all necessary environment variables, we are ready to run the script:

   ```shell
   ./api/scripts/full-script.sh --prod-uuid <PROD-ACCOUNT-UUID> --dev-uuid <DEV-ACCOUNT-UUID>
   ```

6. The script should run and output the results to the console. If it exits early, you should be able to tell by how far the output goes. If you haven't logged in recently via the AWS CLI, you'll be prompted to do so, as well.

7. Once you're done assuming the prod user, don't forget to reset the dev user's data:
