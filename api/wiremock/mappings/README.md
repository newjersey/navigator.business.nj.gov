...
2. **Reset to success** (set state back to "Started"):
   ...
   curl -X PUT http://localhost:9000/__admin/scenarios/GovDeliveryUnsubscribe/state -d '{"state": "Started"}'
   curl -X PUT http://localhost:9000/__admin/scenarios/GovDeliveryUpdateEmail/state -d '{"state": "Started"}'
   ```
