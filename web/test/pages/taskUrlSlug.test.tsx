...
describe("task page", () => {
  ...
  it("shows loading state when roadmap is undefined", () => {
    ...
    render(
      <WithStatefulUserData
        initialUserData={generateUserDataForBusiness(generateBusiness({}))}
      >
        <TaskPage
          task={generateTask({})}
          displayContent={createEmptyTaskDisplayContent()}
  ...
  it("shows updated content if different from static content", () => {
    ...
    expect(
      screen.getByText("a whole brand new description")
    ).toBeInTheDocument();

    expect(
      screen.queryByText("original call to action")
    ).not.toBeInTheDocument();
    expect(
      screen.getByText("a whole brand new call to action")
    ).toBeInTheDocument();
  ...
  it("displays form in task footer when defined value", () => {
    ...
    expect(
      screen.getByText(`${Config.taskDefaults.formNameText}:`)
    ).toBeInTheDocument();
  ...
  it("does not display agency in task footer when agencyId and agencyAdditionalContext are undefined", () => {
    renderPage(
      generateTask({ agencyId: undefined, agencyAdditionalContext: undefined })
    );
    expect(
      screen.queryByText(`${Config.taskDefaults.issuingAgencyText}:`)
    ).not.toBeInTheDocument();
  });
  ...
  it("does not display form name in task footer when it is undefined value", () => {
    ...
    expect(
      screen.queryByText(`${Config.taskDefaults.formNameText}:`)
    ).not.toBeInTheDocument();
  });
  ...
  it("displays agencyId as its name when it is defined", () => {
    ...
    expect(
      screen.getByText(`${Config.taskDefaults.issuingAgencyText}:`)
    ).toBeInTheDocument();
    expect(
      screen.getByText(LookupTaskAgencyById("nj-consumer-affairs").name)
    ).toBeInTheDocument();
  ...
  describe("License task", () => {
    ...
    it("mockTaskIdsWithLicenseSearch matches taskIdsWithLicenseSearchEnabled", () => {
      expect(mockTaskIdsWithLicenseSearch.length).toEqual(
        taskIdsWithLicenseSearchEnabled.length
      );

      expect(mockTaskIdsWithLicenseSearch).toEqual(
        taskIdsWithLicenseSearchEnabled
      );
    });

    it.each(mockTaskIdsWithLicenseSearch)(
      "loads License task screen for %s",
      (licenseId) => {
        renderPage(
  ...
  describe("next and previous task buttons", () => {
    ...
    it("renders only Next Task button for first task", () => {
      renderPage(taskOne);
      expect(
        screen.getByText(Config.taskDefaults.nextTaskButtonText)
      ).toBeInTheDocument();
      expect(
    ...
    it("renders only Previous Task button at last task", () => {
      ...
      expect(
        screen.queryByText(Config.taskDefaults.nextTaskButtonText)
      ).not.toBeInTheDocument();
      expect(
        screen.getByText(Config.taskDefaults.previousTaskButtonText)
      ).toBeInTheDocument();
    });
  ...
  describe("unlocked-by tasks", () => {
    const doThisFirstTask = generateTask({
      urlSlug: "do-this-first",
      stepNumber: 1,
    });
    const alsoThisOneTask = generateTask({
      urlSlug: "also-this-one",
      stepNumber: 1,
    });
    ...
    it("does not show an alert when this task has nothing that it is unlocked by", () => {
      ...
      renderPage(task);
      expect(
        screen.queryByText(Config.taskDefaults.unlockedBySingular, {
          exact: false,
        })
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(Config.taskDefaults.unlockedByPlural, {
          exact: false,
        })
      ).not.toBeInTheDocument();
    });
    ...
    it("shows an alert with link when this task is unlocked by one other task", () => {
      ...
      expect(
        screen.getByText("Do this first", { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText("Do this first", { exact: false })
      ).toHaveAttribute("href", "do-this-first");
    });
    ...
    it("shows an alert with links when this task is unlocked by several other tasks", () => {
      ...
      expect(
        screen.getByText("Also this one", { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText("Do this first", { exact: false })
      ).toHaveAttribute("href", "do-this-first");
      expect(
        screen.getByText("Also this one", { exact: false })
      ).toHaveAttribute("href", "also-this-one");
    });
    ...
    it("ignores unlocked-by tasks on the page's default task", () => {
      ...
      expect(
        screen.queryByText("NOT ON ROADMAP TASK", { exact: false })
      ).not.toBeInTheDocument();
    });
    ...
    it("removes a task from the unlocked-by alert when its status is completed", () => {
      ...
      renderPage(
        task,
        generateBusiness({ taskProgress: { "do-this-first": "COMPLETED" } })
      );
      ...
      expect(
        screen.queryByText("Do this first", { exact: false })
      ).not.toBeInTheDocument();
      expect(
        screen.getByText("Also this one", { exact: false })
      ).toBeInTheDocument();
  ...
  it("does not render next and previous buttons for STARTING when legal structure allows for business formation and form-business-entity task is rendered", () => {
    ...
    expect(
      screen.queryByTestId("nextAndPreviousButtons")
    ).not.toBeInTheDocument();
  });
  ...
  describe("deferred location question", () => {
    ...
    it("shows deferred location question if task requiresLocation=true and municipality is undefined", () => {
      ...
      expect(
        screen.queryByTestId("deferred-location-content")
      ).not.toBeInTheDocument();
    });
    ...
    it("shows deferred location content if task requiresLocation=true and user has municipalities set", () => {
      ...
      expect(screen.getByTestId("deferred-location-task")).toBeInTheDocument();
      expect(
        screen.getByTestId("deferred-location-content")
      ).toBeInTheDocument();
    });
    ...
    it("does not show deferred location question if task requiresLocation=false", () => {
      ...
      renderPage(task);
      expect(
        screen.queryByTestId("deferred-location-task")
      ).not.toBeInTheDocument();
    });
    ...
    it("loads deferred location question using keys in template body", async () => {
      ...
      expect(
        screen.queryByText("${beginLocationDependentSection}")
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText("${endLocationDependentSection}")
      ).not.toBeInTheDocument();
    });
  });
});
