...
describe("onboarding - foreign business", () => {
  ...
  describe("Nexus - step 3", () => {
    ...
    it.each(
      industryIdsWithSingleRequiredEssentialQuestion,
    )("prevents user from moving to Step 4 when %s is selected as industry, but essential question is not answered", async (industryId) => {
      const userData = generateTestUserData({
        businessPersona: "FOREIGN",
        foreignBusinessTypeIds: ["employeeOrContractorInNJ"],
    ...
    it.each([
      industryIdsWithSingleRequiredEssentialQuestion,
    ])("allows user to move past Step 3 when you have selected an industry %s and answered the essential question", async (industryId) => {
      const userData = generateTestUserData({
        businessPersona: "FOREIGN",
        foreignBusinessTypeIds: ["employeeOrContractorInNJ"],
        industryId: industryId,
