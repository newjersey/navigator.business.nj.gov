...
describe("profile-foreign", () => {
  ...
  it("does not display the documents tab", () => {
    renderPage({ business: setupBusiness });
    expect(
      screen.getAllByText(Config.profileDefaults.default.profileTabNumbersTitle)
        .length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText(Config.profileDefaults.default.profileTabNoteTitle)
        .length
    ).toBeGreaterThan(0);
    expect(
      screen.getAllByText(Config.profileDefaults.default.profileTabInfoTitle)
        .length
    ).toBeGreaterThan(0);
    expect(
  ...
  describe("Nexus Foreign Business", () => {
    ...
    it("sets homeBasedBusiness value to false when officeInNJ is checked", async () => {
      ...
      fireEvent.click(
        screen.getByRole("checkbox", {
          name: Config.profileDefaults.fields.foreignBusinessTypeIds.default
            .optionContent.officeInNJ,
        })
      );
      clickSave();
      await waitFor(() => {
    ...
    it("displays the out of state business name field", () => {
      ...
      expect(
        screen.getByText(
          Config.profileDefaults.fields.nexusBusinessName.default
    ...
    it("does not display out-of-state business name when SP/GP", () => {
      ...
      expect(
        screen.queryByText(
          Config.profileDefaults.fields.nexusDbaName.default.header
        )
      ).not.toBeInTheDocument();
    });
    ...
    it("displays the user's business name if they have one", () => {
      ...
      expect(
        within(screen.getByTestId("main")).getByText("Test Business")
      ).toBeInTheDocument();
    });
    ...
    it("renders locked profile address fields when business has formed", () => {
      ...
      expect(
        screen.getByTestId("locked-profileAddressCityStateZip")
      ).toBeInTheDocument();
    });
    ...
    describe("ProfileDateOfFormation", () => {
      ...
      it("updates the date of formation to a future date", () => {
        ...
        expect(
          screen.queryByText(
            Config.profileDefaults.fields.dateOfFormation.default
              .errorTextRequired
          )
        ).not.toBeInTheDocument();
      });
        ...
        renderPage({
          business: generateBusinessForProfile({
            profileData: generateProfileData({
              businessPersona: "FOREIGN",
              foreignBusinessTypeIds: [
                foreignBusinessTypeId as ForeignBusinessTypeId,
              ],
            }),
          }),
        });
