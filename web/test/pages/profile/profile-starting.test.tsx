...
describe("profile - starting business", () => {
  ...
  it("prevents user from saving if they partially entered Employer Id", async () => {
    ...
    expect(screen.getByTestId("profile-header-alert")).toBeInTheDocument();
  });
  ...
  describe("business name not required", () => {
    ...
    for (const phase of allOperatingPhases) {
      it(`allows user to save with empty business name in ${phase} phase`, async () => {
        ...
        expect(screen.queryByTestId("profile-header-alert")).not.toBeInTheDocument();
      });
    }
  });
  ...
  describe("Document Section", () => {
    ...
    it("shows document links", () => {
      ...
      expect(
        screen.getByText(Config.profileDefaults.default.certificationDocFileTitle),
      ).toBeInTheDocument();
      expect(
  ...
  describe("address data", () => {
    it("displays alert when address line 1 is filled then blurred", async () => {
      ...
      expect(
        within(profileAlert).getByText(Config.formation.fields.addressCity.label),
      ).toBeInTheDocument();
      expect(
