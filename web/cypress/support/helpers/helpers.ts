...
export const fillOutLicenseStatusCheckForm = ({
  ...
  if (businessName) {
    // React 19: Use selectall+backspace then typing and blur to ensure state updates complete
    cy.get('[data-testid="business-name"]')
      .focus()
      .type("{selectall}{backspace}", { force: true })
      .type(businessName, { delay: 100, force: true })
      .blur({ force: true });
    cy.wait(500); // Wait for React 19 batched updates
  }

  if (addressLine1) {
    // React 19: Use selectall+backspace then typing and blur to ensure state updates complete
    cy.get('[data-testid="address-1"]')
      .focus()
      .type("{selectall}{backspace}", { force: true })
      .type(addressLine1, { delay: 100, force: true })
      .blur({ force: true });
  ...
  if (addressLine2) {
    // React 19: Use selectall+backspace then typing and blur to ensure state updates complete
    cy.get('[data-testid="address-2"]')
      .focus()
      .type("{selectall}{backspace}", { force: true })
      .type(addressLine2, { delay: 100, force: true })
      .blur({ force: true });
    cy.wait(500);
  }
  ...
  // React 19: Final wait to ensure all state updates have completed before form submission
  cy.wait(1000);
};
