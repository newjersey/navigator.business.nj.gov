...
describe("<LandingPageTiles />", () => {
  ...
  it("routes user to industry selection when the starting a business button is clicked", async () => {
    ...
    fireEvent.click(
      screen.getByText(Config.landingPage.landingPageRegisterBizTile)
    );
    ...
    expect(
      mockAnalytics.event.landing_page_get_my_registration_guide_tile.click
        .go_to_onboarding
  ...
  it("routes user to out-of-state business section when the out-of-state business button is clicked", async () => {
    ...
    fireEvent.click(
      screen.getByText(Config.landingPage.landingPageOutOfStateTile)
    );
    ...
    expect(
      mockAnalytics.event.landing_page_im_an_out_of_business_tile.click
        .go_to_onboarding
    ).toHaveBeenCalledTimes(1);
  });
  ...
  it("routes user to business status section when the running a business button is clicked", async () => {
    ...
    expect(
      mockAnalytics.event.landing_page_file_and_pay_my_taxes_tile.click
        .go_to_onboarding
    ).toHaveBeenCalledTimes(1);
  });
  ...
  it("routes user to business status section when the fundings a business button is clicked", async () => {
    ...
    fireEvent.click(
      screen.getByText(Config.landingPage.landingPageFundingTile)
    );
    ...
    expect(
      mockAnalytics.event.landing_page_find_funding_for_my_business_tile.click
        .go_to_onboarding
    ).toHaveBeenCalledTimes(1);
  });
  ...
  it("routes user to industry selection section when the start business a business button is clicked", async () => {
    ...
    fireEvent.click(
      screen.getByText(Config.landingPage.landingPageStartBizTile)
    );
    ...
    expect(
      mockAnalytics.event.landing_page_im_starting_a_nj_business_tile.click
        .go_to_onboarding
    ).toHaveBeenCalledTimes(1);
  });
  ...
  it("routes user to business status section when the I'm running a business button is clicked", async () => {
    ...
    expect(
      mockAnalytics.event.landing_page_im_running_a_nj_business_tile.click
        .go_to_onboarding
    ).toHaveBeenCalledTimes(1);
  });
});
