...
export const GenericButton = forwardRef(function GenericButton(
  ...
  const noRightMargin = props.isRightMarginRemoved
    ? "margin-right-0"
    : "margin-right-2";
  const fullWidth = props.isFullWidthOnDesktop ? "width-100 flex fjc" : "";
  const isNotFullWidthOnMobile = props.isNotFullWidthOnMobile
    ? "width-auto"
    : "";
  ...
  return (
    <button
      ...
      {...(props.isAriaControls
        ? { "aria-controls": props.isAriaControls }
        : {})}
      {...(props.isAriaExpanded
        ? { "aria-expanded": props.isAriaExpanded }
        : {})}
      {...(props.isAriaHaspopup
        ? { "aria-haspopup": props.isAriaHaspopup }
        : {})}
    >
      {props.isLoading ? (
        <div
