...
describe("<NeedsAccount Modal />", () => {
  ...
  it("shows Needs Account modal when user is in guest mode", () => {
    ...
    expect(
      screen.getByText(Config.selfRegistration.needsAccountModalBody)
    ).toBeInTheDocument();
  });
  ...
  it("routes to account setup when link is clicked", async () => {
    setupHookWithAuth(IsAuthenticated.FALSE);
    fireEvent.click(
      screen.getByText(Config.selfRegistration.needsAccountModalButtonText)
    );
  ...
  it("goes to myNJ when Log-in link is clicked", () => {
    ...
    fireEvent.click(
      screen.getByText(
        markdownToText(Config.selfRegistration.needsAccountModalSubText)
      )
    );
