...
export const ProfileMenuItem = (props: {
  ...
  const hasMultipleBusinesses =
    Object.values(userData.businesses).filter((b) => b.dateDeletedISO===undefined || b.dateDeletedISO==="").length > 1;

  return orderBusinessIdsByDateCreated(userData).filter(businessId => userData.businesses[businessId].dateDeletedISO === undefined || userData.businesses[businessId].dateDeletedISO === "" || userData.businesses[businessId].dateDeletedISO === "").flatMap((businessId, i) => {
