...
describe("OpportunityCard", () => {
  ...
  describe("content truncation based on OPPORTUNITY_CARD_MAX_BODY_CHARS", () => {
    it(`displays ellipsis and first ${
      ...
      const opportunity = generateOpportunity({
        sidebarCardBodyText: characters,
      });
      render(<OpportunityCard opportunity={opportunity} urlPath="funding" />);
    ...
    it(`displays the entire content when character count is ${OPPORTUNITY_CARD_MAX_BODY_CHARS}`, () => {
      const characters = Array(OPPORTUNITY_CARD_MAX_BODY_CHARS)
        .fill("a")
        .join("");
  ...
  it("fires hide_card analytics when hide button is clicked", () => {
    render(
      <OpportunityCard
        opportunity={generateOpportunity({ id: "123" })}
        urlPath="funding"
      />
    );
    fireEvent.click(
      screen.getByText(Config.dashboardDefaults.hideOpportunityText)
    );
    expect(
      mockAnalytics.event.for_you_card_hide_button.click.hide_card
    ).toHaveBeenCalledTimes(1);
    expect(
      mockAnalytics.event.for_you_card_unhide_button.click.unhide_card
  ...
  it("fires unhide_card analytics when unhide button is clicked", () => {
    useMockBusiness({
      preferences: generatePreferences({ hiddenFundingIds: ["123"] }),
    });
    render(
      <OpportunityCard
        opportunity={generateOpportunity({ id: "123" })}
    ...
    fireEvent.click(
      screen.getByText(Config.dashboardDefaults.unHideOpportunityText)
    );
    expect(
      mockAnalytics.event.for_you_card_hide_button.click.hide_card
    ).not.toHaveBeenCalled();
