...
describe("ActivitiesForm", () => {
  ...
  const fillForm = (): void => {
    fireEvent.change(screen.getByLabelText(Config?.crtkTask?.naicsCodeLabel), {
      target: { value: validNaicsCode },
    });
    fireEvent.change(screen.getByLabelText(Config?.crtkTask?.businessActivitiesLabel), {
      target: { value: businessActivitiesText },
    });
    fireEvent.change(screen.getByLabelText(Config?.crtkTask?.materialsLabel), {
      target: { value: materialOrProductsText },
    });
  };
