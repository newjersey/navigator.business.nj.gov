...
import { emptyAddressData } from "@businessnjgovnavigator/shared/formationData";
...
describe("<ProfileAddress  />", () => {
  ...
  const renderNjBusinessComponent = ({ municipalities }: { municipalities?: Municipality[] }): void => {
    render(
  ...
  const renderNonNjUSBusinessComponent = (): void => {
    render(
      <ThemeProvider theme={createTheme()}>
        <WithStatefulAddressData initialData={emptyAddressData} municipalities={[generateMunicipality({})]}>
  ...
  const renderIntlBusinessComponent = (): void => {
    render(
      <ThemeProvider theme={createTheme()}>
        <WithStatefulAddressData initialData={emptyAddressData} municipalities={[generateMunicipality({})]}>
  ...
  describe("NJ business location", () => {
    it("renders profile address line 1 field with input value", () => {
      ...
      renderNjBusinessComponent({});
      expect(screen.getByLabelText("Address line1")).toHaveValue("1111 Home Alone");
    });

    it("renders profile address line 1 reflects changed value", () => {
      const address = generateAddress({
        addressLine1: "",
      });
      useMockBusiness(
  ...
  describe("non-NJ business location", () => {
    ...
    describe("US business", () => {
      it("displays profile address fields for a US business", () => {
        ...
        renderNonNjUSBusinessComponent();
