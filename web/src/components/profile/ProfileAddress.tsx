...
interface Props {
  businessLocationType: FormationBusinessLocationType;
  setBusinessLocationType?: Dispatch<SetStateAction<FormationBusinessLocationType>>;
}

export const ProfileAddress = (props: Props): ReactElement => {
  ...
  const [addressProfile, setAddressProfile] = useState<FormationBusinessLocationType>(
    props.businessLocationType
  );

  const handleAddressTypeChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    setAddressProfile(event.target.value as FormationBusinessLocationType);
    if (props.setBusinessLocationType)
      props.setBusinessLocationType(event.target.value as FormationBusinessLocationType);
  ...
  const onValidation = (): void => {
    ...
    if (addressProfile === "NJ") setIsValidMunicipality(!doesFieldHaveError("addressMunicipality"));
    if (addressProfile === "US" || addressProfile === "INTL")
      setIsValidCity(!doesFieldHaveError("addressCity"));
    if (addressProfile === "US") setIsValidState(!doesFieldHaveError("addressState"));
    if (addressProfile === "INTL") setIsValidProvince(!doesFieldHaveError("addressProvince"));
    if (addressProfile === "INTL") setIsValidZipCountry(!doesFieldHaveError("addressCountry"));
        ...
        <div>
          ...
          {addressProfile !== "NJ" && (
            <FormControl fullWidth>
              <RadioGroup name="business-address" value={addressProfile} onChange={handleAddressTypeChange}>
                <FormControlLabel
                  labelPlacement="end"
