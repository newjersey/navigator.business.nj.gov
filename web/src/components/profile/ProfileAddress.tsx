...
interface Props {
  businessLocationType: FormationBusinessLocationType | undefined;
  setBusinessLocationType?: Dispatch<SetStateAction<FormationBusinessLocationType | undefined>>;
}
...
export const ProfileAddress = (props: Props): ReactElement => {
  ...
  const [addressLocation, setAddressLocation] = useState<FormationBusinessLocationType | undefined>(
    props.businessLocationType
  );

  const handleAddressTypeChange = (event: React.ChangeEvent<HTMLInputElement>): void => {
    setAddressLocation(event.target.value as FormationBusinessLocationType);
  ...
  const onValidation = (): void => {
    ...
    if (addressLocation === "NJ") setIsValidMunicipality(!doesFieldHaveError("addressMunicipality"));
    if (addressLocation === "US" || addressLocation === "INTL")
      setIsValidCity(!doesFieldHaveError("addressCity"));
    if (addressLocation === "US") setIsValidState(!doesFieldHaveError("addressState"));
    if (addressLocation === "INTL") setIsValidProvince(!doesFieldHaveError("addressProvince"));
    if (addressLocation === "INTL") setIsValidZipCountry(!doesFieldHaveError("addressCountry"));
