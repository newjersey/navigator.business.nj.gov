...
export const ProfileAddress = (): ReactElement => {
  ...
  const onValidation = (): void => {
    ...
    if (state.addressData.businessLocationType === "NJ")
      setIsValidMunicipality(!doesFieldHaveError("addressMunicipality"));
    if (state.addressData.businessLocationType === "US" || state.addressData.businessLocationType === "INTL")
      setIsValidCity(!doesFieldHaveError("addressCity"));
    if (state.addressData.businessLocationType === "US") setIsValidState(!doesFieldHaveError("addressState"));
    if (state.addressData.businessLocationType === "INTL") {
  ...
  const showAddressFields = (): ReactElement => {
    switch (state.addressData.businessLocationType) {
      case "INTL":
        return (
          <>
            {addressLines1And2()}
    ...
    <>
      {isAddressFieldsDisabled ? (
        ...
        <div>
          ...
          {state.addressData.businessLocationType !== "NJ" && (
            <FormControl fullWidth>
              <RadioGroup
                name="business-address"
                value={state.addressData.businessLocationType ?? ""}
