...
describe("<CheckEnvPermits />", () => {
  ...
  describe("land", () => {
    it("displays the results page when the user submits the questionnaire", async () => {
      ...
      await waitFor(() => {
        expect(
          screen.getByText(Config.envResultsPage.title)
        ).toBeInTheDocument();
      });
    });
  ...
  it.each([
    ...
    (mediaArea: string, taskId: string) => {
      ...
      expect(
        screen.getByText(Config.envQuestionPage.generic.title)
      ).toBeInTheDocument();
    }
  );
  ...
  it.each([
    ...
    "renders summary when %s task and is mobile",
    (_, taskId: string, name: string) => {
      isMobile(true);
  ...
  it.each([
    ...
    "doesn't render summary when %s task and is not mobile",
    (_, taskId: string, name: string) => {
      isMobile(false);
      const task = generateTask({ id: taskId, name });
      renderQuestionnaire({
      ...
      expect(
        screen.queryByText(task.summaryDescriptionMd)
