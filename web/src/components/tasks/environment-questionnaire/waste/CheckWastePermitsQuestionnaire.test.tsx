...
describe("<CheckWastePermitsQuestionnaire />", () => {
  ...
  it("shows the registration modal if the user in not authenticated", async () => {
    ...
    await user.click(
      screen.getByLabelText(Config.envReqQuestionsPage.waste.questionnaireOptions.hazardousMedicalWaste)
    );
    await waitFor(() => {
  ...
  it("clears all choices if the user selects none of the above", async () => {
    ...
    await user.click(screen.getByText(Config.envReqQuestionsPage.waste.questionnaireOptions.noWaste));
    const generateHazardousWaste: HTMLInputElement = screen.getByLabelText(
      Config.envReqQuestionsPage.waste.questionnaireOptions.hazardousMedicalWaste
    );
  ...
  it("clears none of the above if another selection is made", async () => {
    const { user } = renderQuestionnaireAndSetupUser();
    await user.click(screen.getByLabelText(Config.envReqQuestionsPage.waste.questionnaireOptions.noWaste));
    await user.click(
      screen.getByLabelText(Config.envReqQuestionsPage.waste.questionnaireOptions.hazardousMedicalWaste)
    );
    const noWaste: HTMLInputElement = screen.getByLabelText(
      Config.envReqQuestionsPage.waste.questionnaireOptions.noWaste
    );
