...
import { XrayTabOne } from "@/components/tasks/xray/XrayTabOne";
import { XrayTabZero } from "@/components/tasks/xray/XrayTabZero";

import * as api from "@/lib/api-client/apiClient";
import { useConfig } from "@/lib/data-hooks/useConfig";
import { useRoadmap } from "@/lib/data-hooks/useRoadmap";
import { useUserData } from "@/lib/data-hooks/useUserData";
import { RenewalEventType, Task } from "@/lib/types/types";
...
export const Xray = (props: Props): ReactElement => {
  ...
  const xrayContentFromMarkdown = (): {
    ...
    return { summaryMd, contentMd, callToActionLink };
  };

  const xrayContent = xrayContentFromMarkdown();

  const xrayConfig = props.task ? Config.xrayRegistrationTask : Config.xrayRenewal;
          ...
          <TabContext value={tabIndex.toString()}>
            <Box>
              <TabList
                ...
                aria-label={xrayConfig.ariaLabel}
                sx={{
                  borderBottom: 1,
                  borderColor: "divider",
                  marginTop: ".25rem",
              ...
              >
                <Tab
                  value="0"
                  sx={tabStyle}
                  label={xrayConfig.tab1Text}
                  data-testid={"start-application-tab"}
                />
                <Tab
                  value="1"
                  sx={tabStyle}
                  label={xrayConfig.tab2Text}
            ...
            <TabPanel value="0">
              <XrayTabZero
                summaryDescriptionMd={xrayContent.summaryMd}
                contentMd={xrayContent.contentMd}
                ctaPrimaryText={xrayConfig.callToActionPrimaryText}
                ctaPrimaryOnClick={(): void => {
                  openInNewTab(xrayContent.callToActionLink);
                }}
                ctaSecondaryText={xrayConfig.callToActionSecondaryText}
