...
describe("<RaffleBingoPaginator />", () => {
  ...
  it("renders first tab content when the task is opened", async () => {
    ...
    expect(
      screen.getByRole("button", { name: Config.taskDefaults.continueButtonText }),
    ).toBeInTheDocument();
    expect(
  ...
  it("displays second step task content when on second step", async () => {
    ...
    const stepperButton = await screen.findByTestId("stepper-1");
    await userEvent.click(stepperButton);
  ...
  it("navigates from step two to step one when the back button is clicked", async () => {
    ...
    const backButton = screen.getByRole("button", { name: Config.taskDefaults.backButtonText });
