...
describe("Formation - ContactsStep", () => {
  ...
  describe("when llp", () => {
    const legalStructureId: FormationLegalType =
    ...
    it("auto-fills fields from userData if it exists", async () => {
      ...
      expect(
        screen.getByText(Config.formation.fields.signers.label)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          Config.formation.fields.signers.overrides[
            "limited-liability-partnership"
          ].description
        )
      ).toBeInTheDocument();
  ...
  describe("when lp", () => {
    ...
    it("auto-fills fields from userData if it exists", async () => {
      ...
      expect(
        screen.getByText(Config.formation.fields.incorporators.label)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          Config.formation.fields.incorporators.overrides[
            "limited-partnership"
          ].description.split("\n\n")[0]
        )
      ).toBeInTheDocument();
      ...
      expect(
        screen.queryByText(Config.formation.fields.incorporators.placeholder)
      ).not.toBeInTheDocument();

      expect(screen.getByText("Ava Curie")).toBeInTheDocument();
      expect(
        screen.getByText("160 Something Ave", { exact: false })
      ).toBeInTheDocument();
      expect(screen.getByText("Apt 1", { exact: false })).toBeInTheDocument();
      expect(screen.getByText("Miami", { exact: false })).toBeInTheDocument();
      expect(screen.getByTestId("incorporators-0")).toHaveTextContent(
        "Florida"
      );
  ...
  describe("when corp", () => {
    ...
    it("auto-fills fields from userData if it exists", async () => {
      ...
      expect(
        screen.getByText(
          Config.formation.fields.directors.description.split("\n\n")[0]
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.queryByText(Config.formation.fields.directors.placeholder)
      ).not.toBeInTheDocument();
      expect(screen.getByText(members[0].name)).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressLine1, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressLine2, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressCity as string, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressState!.name, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressZipCode, { exact: false })
      ).toBeInTheDocument();
      ...
      expect(
        screen.getByText(
          Config.formation.fields.incorporators.description.split("\n\n")[0]
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.getByText(incorporators[0].addressLine1, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(incorporators[0].addressLine2, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(incorporators[0].addressCity as string, {
          exact: false,
        })
      ).toBeInTheDocument();
      expect(
        screen.getByText(incorporators[0].addressState!.name, { exact: false })
      ).toBeInTheDocument();
      expect(
        screen.getByText(incorporators[0].addressZipCode, { exact: false })
      ).toBeInTheDocument();
      expect(page.getSignerBox(0, "incorporators")).toEqual(true);
    });
  });
  ...
  describe("when nonprofit", () => {
    ...
    it("auto-fills fields from userData if it exists", async () => {
      ...
      expect(screen.getByTestId("addresses-members")).toBeInTheDocument();
      expect(
        screen.getByText(Config.formation.fields.trustees.label)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          Config.formation.fields.trustees.description.split("\n\n")[0]
        )
      ).toBeInTheDocument();
      expect(
        screen.getByText(
      ...
      expect(
        screen.queryByText(Config.formation.fields.trustees.placeholder)
      ).not.toBeInTheDocument();
      expect(screen.getByText(members[0].name)).toBeInTheDocument();
      expect(
        screen.getByText(members[0].addressLine1, { exact: false })
    ...
    it("has an error if there are no trustees", async () => {
      ...
      expect(
        screen.getByText(Config.formation.fields.trustees.error)
      ).toBeInTheDocument();
    });
    ...
    it("has no error if at least 3 trustees", async () => {
      ...
      await attemptApiSubmission(page);
      expect(
        screen.queryByText(Config.formation.fields.trustees.error)
      ).not.toBeInTheDocument();
    });
  });
  ...
  describe("when flc", () => {
    ...
    it("auto-fills fields from userData if it exists", async () => {
      ...
      expect(page.getInputElementByLabel("Signer 0").value).toBe("signer 1");
      expect(page.getInputElementByTestId("signer-title-0").value).toBe(
        "Authorized Representative"
      );
      expect(page.getInputElementByLabel("Signer 1").value).toBe("signer 2");
      expect(page.getInputElementByTestId("signer-title-1").value).toBe(
        "General Partner"
      );
      expect(page.getInputElementByLabel("Signer 2").value).toBe("signer 3");
      expect(page.getInputElementByTestId("signer-title-2").value).toBe(
        "Authorized Representative"
      );
    });
  });
