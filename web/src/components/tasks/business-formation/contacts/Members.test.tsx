  ...
  describe(`shared behaviors`, () => {
    legalStructureIds.map((legalStructureId) => {
      ...
      const successBodyText =
        Config.formation.fields[configField].successSnackbarBody;
      ...
      describe(`for ${legalStructureId}`, () => {
        ...
        it("does not show members section for foreign legal structures", async () => {
          ...
          expect(
            screen.queryByTestId("addresses-members")
        ...
        it("edits members", async () => {
          ...
          expect(
            screen.getByText(nextButtonText, { exact: false })
          ).toBeInTheDocument();
          ...
          await waitFor(() => {
            expect(
              screen.getByText(successBodyText, { exact: false })
            ).toBeInTheDocument();
          });
          expect(
            screen.getByText(newName, { exact: false })
          ).toBeInTheDocument();
          await page.submitContactsStep();
          const newMembers =
            currentBusiness().formationData.formationFormData.members;
        ...
        it("shows validation on submit", async () => {
          ...
          expect(
            screen.queryByText(addressLine1.error)
          ).not.toBeInTheDocument();
          expect(screen.queryByText(addressCity.error)).not.toBeInTheDocument();
          expect(
            screen.queryByText(addressState.error)
          ).not.toBeInTheDocument();
          expect(
            screen.queryByText(addressZipCode.error)
          ).not.toBeInTheDocument();
          page.clickAddressSubmit();
          await waitFor(() => {
            expect(
        ...
        it("shows maximum length validation for address and city fields on submit", async () => {
          ...
          expect(
            screen.getByText(maxLengthMessage("addressLine1", "35"))
          ).toBeInTheDocument();
          expect(
            screen.getByText(maxLengthMessage("addressLine2", "35"))
          ).toBeInTheDocument();
          expect(
            screen.getByText(maxLengthMessage("addressCity", "30"))
          ).toBeInTheDocument();
          ...
          expect(
            screen.queryByText(maxLengthMessage("addressLine1", "35"))
          ).not.toBeInTheDocument();
          expect(
            screen.queryByText(maxLengthMessage("addressLine2", "35"))
          ).not.toBeInTheDocument();
          expect(
            screen.queryByText(maxLengthMessage("addressCity", "30"))
          ).not.toBeInTheDocument();
        ...
        it("resets form on cancel", async () => {
          ...
          fireEvent.click(
            screen.getByText(Config.formation.general.backButtonText)
          );
          await waitFor(() => {
            return expect(
        ...
        it("does not add more than 10 members", async () => {
          ...
          expect(
            screen.queryByText(nextButtonText, { exact: false })
          ).not.toBeInTheDocument();
          await page.submitContactsStep();
          expect(
            currentBusiness().formationData.formationFormData.members?.length
          ).toEqual(10);
        });
        ...
        it("renders members table", async () => {
          ...
          expect(
            screen.getByTestId("addresses-members-table")
          ).toBeInTheDocument();
        });
      });
    });
    ...
    describe(`default address checkbox`, () => {
      ...
      it("saves and displays municipality name, not displayName on checkbox import", async () => {
        ...
        await waitFor(() => {
          expect(
            screen.getByText(
              Config.formation.fields.members.successSnackbarBody
            )
          ).toBeInTheDocument();
        });
        await page.submitContactsStep();
      ...
      it("only fills & disables required fields with values when checkbox checked", async () => {
        ...
        expect(page.getInputElementByLabel("Address line2").value).toBe("");
        expect(page.getInputElementByLabel("Address zip code").value).toBe("");
        expect(page.getInputElementByLabel("Address line1").disabled).toBe(
          true
        );
        expect(page.getInputElementByLabel("Address line2").disabled).toEqual(
          true
        );
        expect(
          page.getInputElementByLabel("Address zip code").disabled
        ).toEqual(false);
      });
      ...
      it("unselects checkbox when interacting with non-disabled fields", async () => {
        ...
        expect(page.getInputElementByLabel("Address city").disabled).toBe(
          false
        );
        expect(page.getInputElementByLabel("Address state").disabled).toBe(
          false
        );
        expect(
          page.getInputElementByLabel("Address zip code").disabled
  ...
  describe("s-corp and c-corp", () => {
    ...
    for (const legalStructureId of legalStructures) {
      it(`requires directors for ${legalStructureId}`, async () => {
        ...
        expect(
          screen.getByText(Config.formation.fields.directors.error)
        ).toBeInTheDocument();
      });
      ...
      it(`displays directors content for ${legalStructureId}`, async () => {
        ...
        expect(
          screen.getByText(Config.formation.fields.directors.label)
        ).toBeInTheDocument();
        expect(
          screen.queryByText(Config.formation.fields.members.label)
        ).not.toBeInTheDocument();
      });
    }
  });
  ...
  describe("non-corp members legal structures", () => {
    ...
    for (const legalStructureId of legalStructures) {
      it(`does not require members for ${legalStructureId}`, async () => {
        ...
        await attemptApiSubmission(page);
        expect(
          screen.queryByText(Config.formation.fields.directors.error)
        ).not.toBeInTheDocument();
      });
      ...
      it(`displays members content for ${legalStructureId}`, async () => {
        ...
        await page.stepperClickToContactsStep();
        expect(
          screen.getByText(Config.formation.fields.members.label)
        ).toBeInTheDocument();
        expect(
          screen.getByText(Config.formation.fields.members.subheader)
        ).toBeInTheDocument();
        expect(
          screen.queryByText(Config.formation.fields.directors.label)
        ).not.toBeInTheDocument();
      });
    }
  });
