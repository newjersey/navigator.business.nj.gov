...
describe("Formation - Signatures", () => {
  ...
  describe("starting legal structures", () => {
    it.each([
      ...legalStructureIds,
    ])("adds additional signer when legalStructure is %s", async (legalStructureId) => {
    ...
    it.each([
      ...legalStructureIds,
    ])("deletes an additional signer when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper({ legalStructureId }, { signers: [] });
      page.fillText("Signer 0", "Red Skull");
      page.checkSignerBox(0, "signers");
    ...
    it.each([
      ...legalStructureIds,
    ])("does not add more than 9 additional signers when legalStructure is %s", async (legalStructureId) => {
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error to both enter name and sign if both are missing when legalStructure is %s", async (legalStructureId) => {
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error to enter name if only name is missing when legalStructure is %s", async (legalStructureId) => {
      const formationFormData = generateFormationFormData(
        {
          signers: [
            generateFormationSigner({
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error to sign if only signature is missing when legalStructure is %s", async (legalStructureId) => {
      const formationFormData = generateFormationFormData(
        {
          signers: [
            generateFormationSigner({
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when signer name longer than 50 chars when legalStructure is %s", async (legalStructureId) => {
  ...
  describe("foreign legal structures", () => {
    ...
    it.each([
      ...legalStructureIds,
    ])("selects a signer type when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when name, signature, and type is missing when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
        { businessPersona: "FOREIGN", legalStructureId },
        { signers: [] },
      );
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when name is missing for first signer when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
        { businessPersona: "FOREIGN", legalStructureId },
        {
          signers: [
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when name is missing for second signer when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
        { businessPersona: "FOREIGN", legalStructureId },
        {
          signers: [
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when signers do not fill out the type field when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
        { businessPersona: "FOREIGN", legalStructureId },
    ...
    it.each([
      ...legalStructureIds,
    ])("displays inline error when the signature is missing when legalStructure is %s", async (legalStructureId) => {
      const page = await getPageHelper(
        { businessPersona: "FOREIGN", legalStructureId },
        {
          signers: [
