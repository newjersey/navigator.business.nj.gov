    ...
    legalStructureIds.map((legalStructureId) =>
      describe(`for ${legalStructureId}`, () => {
        it("adds signer", async () => {
          ...
          expect(
            currentBusiness().formationData.formationFormData.incorporators
          ).toEqual([{ ...signer, signature: true, title: signer.title }]);
        });
        ...
        it("edits signer", async () => {
          ...
          const nameTd = screen.getByText(incorporators[1].name, {
            exact: false,
          });
          ...
          expect(page.getInputElementByLabel("Address zip code").value).toBe(
            incorporators[1].addressZipCode
          );
        ...
        it("fires validations when signers do not fill out all the fields", async () => {
          ...
          const nameTd = screen.getByText(incorporators[0].addressLine1, {
            exact: false,
          });
        ...
        it("fires validations when signers do not check the sign checkbox", async () => {
          const incorporators = [
            generateFormationIncorporator({}, legalStructureId),
          ];
          const page = await getPageHelper(
            { legalStructureId },
            { incorporators }
