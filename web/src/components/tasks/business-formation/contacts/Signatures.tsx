...
export const Signatures = (): ReactElement => {
  ...
  const getDescription = (): string => {
    ...
    if (overriddenLegalTypes.includes(legalType)) {
      ...
      return (Config.formation.fields.signers.overrides as any)[legalType]
        .description;
    }
    ...
    return (
      ...
      >
        <label
          htmlFor={
            index
              ? `signature-checkbox-signers-${index}`
              : `signature-checkbox-signers`
          }
          className="text-bold"
          style={{ display: "none" }}
        >
        ...
        >
          <Checkbox
            ...
            checked={checked}
            {...(doesFieldHaveError(FIELD_NAME) && !checked
              ? { color: "error" }
              : {})}
          />
        </div>
  ...
  const getTypeField = (index: number): ReactNode => {
    ...
    return (
      <>
        ...
        >
          {BusinessSignerTypeMap[state.formationFormData.legalType].map(
            (title) => {
              return (
        ...
        {hasError &&
          state.formationFormData!.signers[index].title === undefined && (
  ...
  const getSignatureField = (index: number): ReactNode => {
    ...
    const isTooLong = (): boolean =>
      (state.formationFormData.signers &&
        state.formationFormData.signers[index].name.length >
          SIGNER_NAME_MAX_LEN) ??
      false;
        ...
        <FormationField fieldName="signers">
          <GenericTextField
            ...
            handleChange={(value: string): void =>
              handleSignerChange(value, index)
            }
  ...
  const doesRowHaveError = (index: number): boolean => {
    ...
    const needsTypeField =
      BusinessSignerTypeMap[state.formationFormData.legalType].length > 1;
    const error =
      signer.name.length === 0 ||
      !signer.signature ||
      signer.name.length > SIGNER_NAME_MAX_LEN;
    if (needsTypeField) {
                  ...
                  <div className="grid-row margin-top-1">
                    ...
                    {needsSignerType && (
                      <div
                        className={`${
                          isTabletAndUp
                            ? "grid-col-6 padding-x-2"
                            : "grid-col-12 margin-y-2"
                        }`}
                      >
                        <strong>
                          <Content>
                            {Config.formation.fields.signers.titleLabel}
                          </Content>
                        </strong>
                        {getTypeField(0)}
                      </div>
                ...
                >
                  <div className="grid-col">
                    ...
                    >
                      ...
                      <div
                        className="grid-row margin-top-1"
