...
describe("Formation - ReviewStep", () => {
  ...
  const renderStep = async (
    ...
    if (isForeign) {
      await page.fillAndSubmitNexusBusinessNameStep(
        formationFormData.businessName
      );
    }
  ...
  it("displays business name entered in step 1 as part of business designator line", async () => {
    ...
    const designatorSection = within(
      screen.getByTestId("review-suffix-and-start-date")
    );
    expect(
  ...
  it("displays not entered on the business designator line if suffix is undefined", async () => {
    ...
    expect(
      designatorSection.getByText(Config.formation.general.notEntered)
    ).toBeInTheDocument();
  });
  ...
  it("always displays the total share of stock if legal type is corporation", async () => {
    ...
    expect(
      within(legalStockSection).getByText(
  ...
  it("doesn't display the primary business address if user has not inputted anything", async () => {
    ...
    await renderStep(initialProfileData, formationFormData);
    expect(
      screen.queryByText(Config.formation.sections.addressHeader)
    ).not.toBeInTheDocument();
  });
  ...
  it("displays the primary business address if user has inputted any address field", async () => {
    ...
    await renderStep(initialProfileData, formationFormData);
    expect(
      screen.getByText(Config.formation.sections.addressHeader)
    ).toBeInTheDocument();
  });
  ...
  it("does not display foreign date of formation within review step when non-foreign", async () => {
    ...
    expect(
      screen.queryByTestId("foreign-date-of-formation")
    ).not.toBeInTheDocument();
  });
  ...
  it("display foreign state of formation within review step", async () => {
    ...
    expect(
      screen.getByTestId("foreign-state-of-formation")
    ).toBeInTheDocument();
  ...
  it("does not display foreign state of formation within review step when non-foreign", async () => {
    await renderStep({ businessPersona: "STARTING" }, {});
    expect(
      screen.queryByTestId("foreign-state-of-formation")
    ).not.toBeInTheDocument();
  });
  ...
  it("displays the empty incorporators section within review step when incorporators are empty", async () => {
    ...
    expect(
      screen.getByTestId("review-incorporators-not-entered")
    ).toBeInTheDocument();
  });
  ...
  it("displays the empty signers section within review step when signers are empty", async () => {
    ...
    expect(
      screen.getByTestId("review-signers-not-entered")
    ).toBeInTheDocument();
  });
  ...
  it("displays different titles when legalStructure is a ForProfit Corporation", async () => {
    ...
    expect(
      screen.getByText(
        markdownToText(Config.formation.fields.directors.reviewStepHeader)
      )
    ).toBeInTheDocument();
    expect(
      screen.getByText(
        markdownToText(Config.formation.fields.incorporators.label)
      )
    ).toBeInTheDocument();
  });
  ...
  it("does not displays signer titles when domestic", async () => {
    ...
    expect(
      screen.queryByText(Config.formation.fields.signers.titleLabel)
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText("Authorized Partner", { exact: false })
    ).not.toBeInTheDocument();
  });
  ...
  it("displays signer titles when foreign", async () => {
    ...
    expect(
      screen.getByText(`${Config.formation.fields.signers.titleLabel}:`)
    ).toBeInTheDocument();
    expect(
      screen.getByText("Authorized Partner", { exact: false })
    ).toBeInTheDocument();
  });
  ...
  it("displays signer name when it exists in userData", async () => {
    ...
    expect(
      reviewSignersSection.getByText(
        `${Config.formation.addressModal.name.label}:`
      )
    ).toBeInTheDocument();
  });
  ...
  describe("members", () => {
    ...
    const nonTradeNamelegalStructures = LegalStructures.filter(
      (legalStructure) => {
    ...
    const shouldShowMembersSectionNoMembers = (
      legalStructure: string
    ): boolean => {
      if (
        ["nonprofit", "c-corporation", "s-corporation"].includes(legalStructure)
      )
        return true;
      return false;
    };
    ...
    it.each(legalStructureIds)(
      ...
      async (legalStructure) => {
        ...
        if (shouldShowMembersSectionNoMembers(legalStructure)) {
          ...
          /* eslint-disable-next-line jest/no-conditional-expect */
          expect(
            screen.queryByTestId("review-members")
          ).not.toBeInTheDocument();
        }
      }
    );
  ...
  describe("address", () => {
    it("displays the address line 2 field if in userData", async () => {
      ...
      expect(
        screen.getByTestId("business-address-line-two")
      ).toBeInTheDocument();
    });
    ...
    it("does not display the address line 2 field if not in userData", async () => {
      ...
      expect(
        screen.queryByTestId("business-address-line-two")
      ).not.toBeInTheDocument();
    });
    ...
    it("does not include country in address when non-intl", async () => {
      ...
      expect(
        ...
        screen.queryByText(
          arrayOfCountriesObjects.find((cu) => cu.shortCode === "US")!.name,
          {
            exact: false,
          }
        )
    ...
    it("displays province name in address when intl", async () => {
      await renderStep(
        { businessPersona: "FOREIGN" },
        {
          addressProvince: "Random place whatever",
          businessLocationType: "INTL",
        }
      );
      expect(
        screen.getByText("Random place whatever", { exact: false })
      ).toBeInTheDocument();
    });
    ...
    it("displays province name label in address when intl", async () => {
      ...
      expect(
        screen.getByText(Config.formation.fields.addressProvince.label)
      ).toBeInTheDocument();
    });
    ...
    it("displays city name in address when non-nj", async () => {
      ...
      expect(
        screen.getByText("Roflcopterville", { exact: false })
      ).toBeInTheDocument();
    });
    ...
    it("displays Municipality name in address when NJ", async () => {
      ...
      expect(
        screen.getByText("Some city", { exact: false })
      ).toBeInTheDocument();
    });
  });
  ...
  describe("when lp", () => {
    ...
    it("displays withdrawals on review step", async () => {
      await renderStep(
        { legalStructureId },
        { withdrawals: "withdrawal stuff" }
      );
      expect(
        screen.getByText(Config.formation.fields.withdrawals.label)
      ).toBeInTheDocument();
    ...
    it("displays dissolution on review step", async () => {
      await renderStep(
        { legalStructureId },
        { dissolution: "dissolution stuff" }
      );
      expect(
        screen.getByText(Config.formation.fields.dissolution.label)
      ).toBeInTheDocument();
    ...
    it("displays combined-investment on review step", async () => {
      await renderStep(
        { legalStructureId },
        { combinedInvestment: "combinedInvestment stuff" }
      );
      expect(
    ...
    it("displays partnership rights on review step", async () => {
      ...
      expect(
        screen.getByText(Config.formation.partnershipRights.label)
      ).toBeInTheDocument();
      expect(
        getByMarkup(
          markdownToText(
      ...
      expect(
        getByMarkup(
          markdownToText(
            Config.formation.fields.canGetDistribution.reviewStepNo
          )
        )
      ).toBeInTheDocument();
      ...
      expect(
        getByMarkup(
          markdownToText(
            Config.formation.fields.canMakeDistribution.reviewStepYes
          )
        )
      ).toBeInTheDocument();
  ...
  describe("when llc", () => {
    ...
    it("does not display partnership rights on review step", async () => {
      ...
      expect(
        screen.queryByText(Config.formation.partnershipRights.label)
      ).not.toBeInTheDocument();
    });
    ...
    it("does not display withdrawals on review step", async () => {
      await renderStep(
        { legalStructureId },
        { withdrawals: "withdrawl stuff" }
      );
      expect(
        screen.queryByText(Config.formation.fields.withdrawals.label)
      ).not.toBeInTheDocument();
    });
    ...
    it("does not display dissolution on review step", async () => {
      ...
      expect(
        screen.queryByText(Config.formation.fields.dissolution.label)
      ).not.toBeInTheDocument();
    });
  ...
  describe("billing section", () => {
    ...
    it("displays services payment type for ACH", async () => {
      ...
      expect(
        screen.getByText(Config.formation.fields.paymentType.achLabel)
      ).toBeInTheDocument();
      expect(
    ...
    it("displays documents as comma separated list", async () => {
      ...
      const formationDocLabel =
        Config.formation.fields.officialFormationDocument.label;
      const certifiedCopyLabel =
        Config.formation.fields.certifiedCopyOfFormationDocument.label;
      expect(
        screen.getByText(`${formationDocLabel}, ${certifiedCopyLabel}`)
      ).toBeInTheDocument();
    });
    ...
    it("displays Not set for phone number when there is no phone number", async () => {
      ...
      const phoneNumberField = within(
        screen.getByTestId("contact-phone-number-field")
      );
      expect(
        phoneNumberField.getByText(Config.formation.general.notEntered)
      ).toBeInTheDocument();
    });
  });
  ...
  describe("when nonprofit", () => {
    ...
    it("displays Not Entered for Is Veteran Nonprofit when undefined", async () => {
      ...
      expect(
        section.getByText(Config.formation.general.notEntered)
      ).toBeInTheDocument();
    });
    ...
    it("displays Not Entered for board members in the Provisions section when undefined", async () => {
      ...
      const boardMemberSection = within(
        screen.getByTestId("hasNonprofitBoardMembers")
      );
      expect(
        boardMemberSection.getByText(Config.formation.general.notEntered)
      ).toBeInTheDocument();
    });
    ...
    describe.each([
      ...
      {
        radio: "nonprofitBoardMemberRightsSpecified",
        terms: "nonprofitBoardMemberRightsTerms",
      },
      {
        radio: "nonprofitTrusteesMethodSpecified",
        terms: "nonprofitTrusteesMethodTerms",
      },
      {
        radio: "nonprofitAssetDistributionSpecified",
        terms: "nonprofitAssetDistributionTerms",
      },
      ...
      it(`does not display ${args.radio} when no board members`, async () => {
        ...
        expect(
          screen.queryByText(
            ((Config.formation.fields as any)[args.radio] as any).body
          )
        ).not.toBeInTheDocument();
      });
      ...
      it(`displays Not Entered for empty terms when IN_FORM`, async () => {
        ...
        const termsReviewSection = within(
          screen.getByTestId(`${args.radio}-terms`)
        );
        expect(
          termsReviewSection.getByText(Config.formation.general.notEntered)
        ).toBeInTheDocument();
        expect(
          termsReviewSection.getByText(
  ...
  describe("when foreign corporation", () => {
    it("displays foreign cert of good standing", async () => {
      await renderStep(
        { businessPersona: "FOREIGN", legalStructureId: "c-corporation" },
        {}
      );
      expect(
  ...
  describe("when foreign nonprofit", () => {
    it("displays foreign cert of good standing", async () => {
      await renderStep(
        { businessPersona: "FOREIGN", legalStructureId: "nonprofit" },
        {}
      );
      expect(
        screen.getByText(Config.formation.fields.foreignGoodStandingFile.label)
