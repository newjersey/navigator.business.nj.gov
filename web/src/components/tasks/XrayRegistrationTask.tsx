...
import { openInNewTab, useMountEffectWhenDefined } from "@/lib/utils/helpers";
...
export const XrayRegistrationTask = (props: Props): ReactElement => {
      ...
      api
        ...
        .then((response) => {
          const xrayData = {
            ...
            deactivatedDate: response.status === "INACTIVE" ? response.deactivatedDate : undefined,
          };
          updateQueue
            ?.queueXrayRegistrationData(xrayData)
            .queueTaskProgress({ "xray-reg": "COMPLETED" })
            .update();

          setXrayRegistrationData(xrayData);
        })
        .catch((error) => {
          if (error === "NOT_FOUND") {
            setError("NOT_FOUND");
  ...
  useMountEffectWhenDefined(() => {
    if (business?.xrayRegistrationData) {
            ...
            <TabPanel value="1">
              <div className="margin-top-3">
                {xrayRegistrationData ? (
                  <XrayRegistrationSummary xrayRegistrationData={xrayRegistrationData} edit={onEdit} />
                ) : (
