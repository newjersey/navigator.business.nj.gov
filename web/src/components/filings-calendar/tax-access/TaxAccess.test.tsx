...
describe("<TaxAccess />", () => {
  ...
  describe("when poppy or dakota", () => {
    it.each(["STARTING", "FOREIGN"])(
      "shows step 2 only with no step 1 for %s",
      (persona) => {
        renderComponent(
          generateBusiness({
        ...
        expect(
          screen.queryByText(Config.taxAccess.stepOneHeader)
        ).not.toBeInTheDocument();
        expect(
          screen.queryByText(Config.taxAccess.stepTwoHeader)
        ).not.toBeInTheDocument();
        expect(
          screen.queryByText(Config.taxAccess.stepTwoBackButton)
        ).not.toBeInTheDocument();
        expect(
          screen.getByText(Config.taxAccess.taxCalendarAccessBody)
        ).toBeInTheDocument();
  ...
  describe("when legal structure is undefined", () => {
    ...
    it("shows step 1 question to choose a legal structure", () => {
      ...
      expect(
        screen.getByText(Config.taxAccess.stepOneHeader)
      ).toBeInTheDocument();
      expect(
        screen.getByText(Config.taxAccess.taxCalendarAccessBody)
      ).toBeInTheDocument();
    ...
    it("saves selection and moves to step 2 when clicking next button", async () => {
      ...
      expect(currentBusiness().profileData.legalStructureId).toBe(
        "c-corporation"
      );
      await waitFor(() => {
        expect(
          screen.getByText(Config.taxAccess.stepTwoHeader)
        ).toBeInTheDocument();
      });
      expect(
        screen.queryByText(Config.taxAccess.stepOneHeader)
    ...
    it("shows error when blur without selection", () => {
      renderComponent(undefinedLegalStructureBusiness);
      expect(
        screen.queryByText(Config.taxAccess.stepOneErrorBanner)
      ).not.toBeInTheDocument();
      expect(
        screen.queryByText(
          Config.profileDefaults.fields.legalStructureId.default
            .errorTextRequired
        )
      ).not.toBeInTheDocument();
      ...
      expect(
        screen.getByText(Config.taxAccess.stepOneErrorBanner)
      ).toBeInTheDocument();
      expect(
        screen.getByText(
          Config.profileDefaults.fields.legalStructureId.default
  ...
  describe("when legal structure is already defined", () => {
    it("shows step 2 question without steps header", () => {
      ...
      expect(
        screen.queryByLabelText("Business structure")
      ).not.toBeInTheDocument();
    });
  ...
  describe("tax calendar access analytics", () => {
    ...
    it("tracks tax calendar access analytics", () => {
      renderComponent(undefinedLegalStructureBusiness);
      expect(
        mockAnalytics.event.tax_calendar.arrive.arrive_calendar_access_v2
      ).toHaveBeenCalled();
    });
  });
});
