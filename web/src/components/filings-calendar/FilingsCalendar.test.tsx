...
const currentDateMock = (
  getCurrentDateModule as jest.Mocked<typeof getCurrentDateModule>
).getCurrentDate;
...
describe("<FilingsCalendar />", () => {
  ...
  it("displays filings calendar with annual report date", () => {
    ...
    expect(
      screen.getByText(dueDate.format("M/D"), { exact: false })
    ).toBeInTheDocument();
  ...
  it("displays filings only within a year in list view", () => {
    ...
    const business = generateBusiness({
      ...
      taxFilingData: generateTaxFilingData({
        filings: [farReport, recentReport],
      }),
  ...
  it("displays filings nested within a date in list view", () => {
    ...
    const element = screen.getByText(recentDueDate.format("MMMM D, YYYY"), {
      exact: false,
    });
    ...
    expect(
      dateElement.getByText(operateReferences["whatever"].name, {
        exact: false,
      })
    ).toBeInTheDocument();
    expect(
      dateElement.getByText(operateReferences["whatever2"].name, {
        exact: false,
      })
    ).toBeInTheDocument();
  });
  ...
  it("displays calendar content when there are filings inside of the year", () => {
    const annualReport = generateTaxFilingCalendarEvent({
      identifier: "annual-report",
      dueDate: getAprilDateOfThisYear()
        .add(2, "months")
    ...
    const business = generateBusiness({
      profileData: generateProfileData({
        operatingPhase: randomElementFromArray(
          OperatingPhases.filter((obj) => {
            return (
              obj.displayCalendarType === "FULL" &&
              obj.displayCalendarToggleButton
            );
          })
        ).id,
      }),
    ...
    renderFilingsCalendar(operateReferences, business);
    expect(
      screen.getByText(Config.dashboardDefaults.calendarListViewButton, {
        exact: false,
      })
    ).toBeInTheDocument();
    expect(screen.getByTestId("filings-calendar-as-table")).toBeInTheDocument();
    expect(
      screen.queryByText(
        markdownToText(Config.dashboardDefaults.emptyCalendarTitleText)
      )
    ).not.toBeInTheDocument();
  });
  ...
  it("displays calendar content when there are filings in two years", () => {
    ...
    fireEvent.click(
      screen.getByTestId("primary-year-selector-dropdown-button")
    );
    fireEvent.click(
      screen.getByText(
  ...
  it("displays empty calendar content when there are filings in two years", () => {
    ...
    fireEvent.click(
      screen.getByText(Config.dashboardDefaults.calendarGridViewButton, {
        exact: false,
      })
    );
    expect(screen.getByTestId("filings-calendar")).not.toHaveTextContent(
  ...
  it("hides year selector when there are no taxFilings", () => {
    ...
    expect(
      screen.queryByTestId("primary-year-selector-dropdown-button")
    ).not.toBeInTheDocument();
  });
  ...
  it("displays empty calendar content when there are no filings", () => {
    ...
    expect(
      screen.queryByTestId("filings-calendar-as-table")
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(Config.dashboardDefaults.calendarListViewButton, {
        exact: false,
      })
    ).not.toBeInTheDocument();
    expect(
  ...
  describe("formation date prompt", () => {
    ...
    it.each(publicFilingLegalStructures)(
      ...
      (legalStructureId) => {
        const business = generateBusiness({
          ...
          taxFilingData: generateTaxFilingData({
            filings: [
              generateTaxFilingCalendarEvent({ identifier: "filing1" }),
            ],
          }),
        });
        const operateReferences: Record<string, OperateReference> = {
    ...
    it.each(publicFilingLegalStructures)(
      ...
      (legalStructureId) => {
        ...
        renderFilingsCalendar({}, business);
        expect(
          screen.queryByTestId("formation-date-prompt")
        ).not.toBeInTheDocument();
      }
    );
  ...
  it("displays filings calendar as list with license events", () => {
    ...
    expect(
      screen.getByText(licenseEvents[0].expirationEventDisplayName)
    ).toBeInTheDocument();
  });
  ...
  it("sends analytics when feedback modal link is clicked", () => {
    ...
    renderFilingsCalendar(operateReferences, business);
    fireEvent.click(
      screen.getByText(Config.dashboardDefaults.calendarFeedbackButtonText)
    );
    expect(
      mockAnalytics.event.share_calendar_feedback.click.open_live_chat
    ).toHaveBeenCalled();
  });
  ...
  describe("tax calendar access button", () => {
    ...
    it("displays button on filings calendar for PublicFiling", () => {
      ...
      const business = generateBusiness({
        profileData: generateProfileData({
          legalStructureId: randomLegalStructure({ requiresPublicFiling: true })
            .id,
          operatingPhase: randomElementFromArray(
            OperatingPhases.filter((obj) => {
              return (
                obj.displayTaxAccessButton === true &&
                obj.displayCalendarType !== "NONE"
              );
            })
          ).id,
        }),
    ...
    it("displays button on filings calendar for TradeName", () => {
      ...
      const business = generateBusiness({
        profileData: generateProfileData({
          legalStructureId: randomLegalStructure({
  ...
  describe("filings calendar in mobile", () => {
    ...
    it("displays filings calendar as list in mobile", () => {
      ...
      expect(
        screen.getByTestId("filings-calendar-as-list")
      ).toBeInTheDocument();
      expect(
        screen.queryByTestId("filings-calendar-as-table")
      ).not.toBeInTheDocument();
    ...
    it("doesn't display past months in list view", () => {
      ...
      const business = generateBusiness({
        ...
        taxFilingData: generateTaxFilingData({
          filings: [pastReport, futureReport],
        }),
      });
  ...
  describe("calendar list and grid views", () => {
    ...
    it("displays calendar list view when button is clicked", () => {
      ...
      expect(
        screen.getByTestId("filings-calendar-as-table")
      ).toBeInTheDocument();
      fireEvent.click(
        screen.getByText(Config.dashboardDefaults.calendarListViewButton, {
          exact: false,
    ...
    it("displays calendar grid view when button is clicked", () => {
      ...
      expect(
        screen.queryByTestId("filings-calendar-as-list")
      ).not.toBeInTheDocument();
    });
  ...
  describe("calendar list view, view more functionality", () => {
    const renderCalendarWithEntries = (
      numberOfCalendarEntries: number
    ): void => {
      let dueDate: dayjs.Dayjs;
      let annualReport: TaxFilingCalendarEvent;
      const operateReferences: Record<string, OperateReference> = {};
