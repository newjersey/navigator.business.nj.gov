...
const updateLicenseDataWithWebflowId = (webflowId: string, filename: string): void => {
...
const createNewLicenseDatas = async (): Promise<void> => {
  ...
  const create = async (licenseMd: LicenseData): Promise<void> => {
    ...
    if (licenseMd.webflowId === undefined && result) {
      ...
      const webflowId = (result.data as WebflowCreateItemResponse).id;
      updateLicenseDataWithWebflowId(webflowId, filename);
    }
    return;
  };
