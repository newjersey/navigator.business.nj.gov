...
const faqToWebflowFormat = (faq: FaqItem): WebflowFaqFieldData => {
  return {
    name: faq.name,
    slug: faq.slug,
...
const getNewFaqs = async (navigatorFaqs: FaqItem[]): Promise<FaqItem[]> => {
  return navigatorFaqs.filter((faq) => !faq.webflowId);
};

const getUnusedFaqs = async (
  navigatorFaqs: FaqItem[],
): Promise<WebflowItem<WebflowFaqFieldData>[]> => {
...
const deleteFaqs = async (navigatorFaqs: FaqItem[]): Promise<void> => {
...
const updateFaqs = async (navigatorFaqs: FaqItem[]): Promise<void> => {
...
const createNewFaqs = async (navigatorFaqs: FaqItem[]): Promise<void> => {
  const newFaqs = await getNewFaqs(navigatorFaqs);

  const create = async (faq: FaqItem): Promise<void> => {
