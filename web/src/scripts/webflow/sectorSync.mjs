...
const getOverlappingSectorsFunc = (currentSectors) =>
  {return currentSectors.filter((item) => {return new Set(getSectors().map((_item) => {return _item.id})).has(item.slug)})};

const getOverlappingSectors = async () => {return getOverlappingSectorsFunc(await getCurrentSectors())};

const getUpdatedSectors = async () => {
  const sectorNames = new Set(getSectors().map((_item) => {return _item.name}));
  return [...(await getOverlappingSectors())].filter((item) => {return !sectorNames.has(item.name)});
};
...
const getNewSectors = async () => {
  ...
  return getSectors()
    ...
    .map((i) => {return { name: i.name, slug: i.id }});
};

const getUnUsedSectors = async () => {
  const current = await getCurrentSectors();
  const overLap = getOverlappingSectorsFunc(current);
  return current.filter((item) => {return !(overLap.includes(item) || item._id == allIndustryId)});
};
const getUpdatedSectorNames = async () => {
  const sectors = getSectors();
  return [...(await getUpdatedSectors())].map((item) => {return {
    ...item,
    name: sectors.find((sector) => {return sector.id == item.slug}).name,
  }});
};

const getSortedSectors = async () => {
  const current = await getCurrentSectors();
  const allIndustryItem = current.find((item) => {return item._id == allIndustryId});
  return [allIndustryItem, ...orderBy(getOverlappingSectorsFunc(current), ["name"], "asc")].map((e, i) => {return {
    ...e,
    rank: i + 1,
  }});
};
...
const deleteSectors = async () =>
  {return [...(await getUnUsedSectors())].map((item) => {return deleteItem(item, sectorCollectionId)})};
...
const createNewSectors = async () =>
  {return [...(await getNewSectors())].map((item) => {return createItem(item, sectorCollectionId, false)})};
...
if (!process.argv.some((i) => {return i.includes("sectorSync")}) || process.env.NODE_ENV == "test") {
...
} else if (process.argv.some((i) => {return i.includes("--previewUpdate")})) {
  await (async () => {
