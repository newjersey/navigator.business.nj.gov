...
jest.mock("../fundingExport", () => ({
  loadAllFundings: jest.fn().mockReturnValue([]),
}));
jest.mock("./helpers", () => ({
  resolveApiPromises: jest.fn().mockResolvedValue(undefined),
  catchRateLimitErrorAndRetry: jest.fn().mockResolvedValue(undefined),
  contentToStrings: jest.fn((str) => str.split("\n")),
  getHtml: jest.fn((lines) => lines.join(" ")),
  wait: jest.fn().mockResolvedValue(undefined),
}));
jest.mock("./methods", () => ({
  getAllItems: jest.fn().mockResolvedValue([]),
  createItem: jest.fn().mockResolvedValue({ data: { id: "new-id" }, headers: new Headers() }),
...
describe("fundingSync", () => {
  ...
  beforeEach(() => {
    ...
    // Mock getAllItems to return different data based on collection ID
    mockedMethods.getAllItems.mockImplementation(async (collectionId: string) => {
      // If it's the sector collection, return sectors
      if (collectionId.includes("sector") || collectionId === "61c21253f7640b5f5ce829a4") {
        return mockSectors as any;
      }
      // Otherwise return fundings
      return mockWebflowFundings as any;
    });
...
`;
  ...
  describe("getNewFundings", () => {
    it("identifies fundings that don't exist in Webflow", async () => {
      ...
      mockedMethods.getAllItems.mockImplementation(async (collectionId: string) => {
        ...
        return [mockWebflowFundings[0]] as any;
      });
  ...
  describe("deleteFundings", () => {
    it("deletes unused fundings", async () => {
      ...
      expect(mockedMethods.deleteItem).toHaveBeenCalledWith(
        mockWebflowFundings[1].id,
        expect.any(String),
      );
    });
