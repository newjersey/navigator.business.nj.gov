...
describe("sectorSync", () => {
  ...
  const mockWebflowSectors: WebflowItem<WebflowSectorFieldData>[] = [
    {
      id: allIndustryId,
    ...
    {
      id: "webflow-1",
      fieldData: { name: "Clean Energy", slug: "clean-energy", rank: 2 },
    },
    {
      id: "webflow-2",
      fieldData: { name: "Old Tech Name", slug: "technology", rank: 3 },
    },
    {
      id: "webflow-3",
      fieldData: { name: "Unused Sector", slug: "unused", rank: 4 },
    },
  ];
  ...
  describe("getSectors", () => {
    it("reads sectors from JSON file", () => {
      ...
      expect(result[0].id).toBe("clean-energy");
      expect(result[0].name).toBe("Clean Energy");
    ...
    it("parses JSON correctly", () => {
      ...
      expect(result[1].id).toBe("technology");
      expect(result[1].name).toBe("Technology");
      expect(result[2].id).toBe("healthcare");
      expect(result[2].name).toBe("Healthcare");
    });
  });
  ...
  describe("getNewSectors", () => {
    it("identifies sectors that don't exist in Webflow", async () => {
      const webflowWithoutHealthcare = mockWebflowSectors.slice(0, 3);
      mockedMethods.getAllItems.mockResolvedValue(webflowWithoutHealthcare);
    ...
    it("returns empty array when all sectors exist", async () => {
      const allSectors = [
        ...
        {
          id: "webflow-4",
          fieldData: { name: "Healthcare", slug: "healthcare" },
        },
      ];
    ...
    it("maps sectors to WebflowSector format", async () => {
      ...
      expect(result[0].slug).toBe("clean-energy");
    });
  });
  ...
  describe("getUnUsedSectors", () => {
    it("identifies sectors in Webflow but not in local JSON", async () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].fieldData.slug).toBe("unused");
    });
    ...
    it("returns empty array when all sectors are used", async () => {
      const usedSectors = mockWebflowSectors.filter((s) => s.fieldData.slug !== "unused");
  ...
  describe("getUpdatedSectorNames", () => {
    it("identifies sectors with name changes", async () => {
      ...
      expect(result).toHaveLength(1);
      expect(result[0].fieldData.slug).toBe("technology");
      expect(result[0].fieldData.name).toBe("Technology");
    });
  ...
  describe("getSortedSectors", () => {
    it("returns sectors sorted alphabetically with allIndustry first", async () => {
      ...
      expect(result[1].fieldData.name).toBe("Clean Energy");
  ...
  describe("deleteSectors", () => {
    it("deletes unused sectors", async () => {
      ...
      expect(mockedMethods.deleteItem).toHaveBeenCalledWith("webflow-3", expect.any(String));
    });
    ...
    it("logs deletion attempts", async () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalledWith("Attempting to delete Unused Sector");
      consoleInfoSpy.mockRestore();
    });

    it("does nothing when no unused sectors", async () => {
      const usedOnly = mockWebflowSectors.filter((s) => s.fieldData.slug !== "unused");
  ...
  describe("updateSectorNames", () => {
    ...
    it("logs update attempts", async () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalledWith("Attempting to modify Technology");
      consoleInfoSpy.mockRestore();
    });
  ...
  describe("createNewSectors", () => {
    it("creates new sectors", async () => {
      const withoutHealthcare = mockWebflowSectors.slice(0, 3);
      mockedMethods.getAllItems.mockResolvedValue(withoutHealthcare);
    ...
    it("logs creation attempts", async () => {
      ...
      expect(consoleInfoSpy).toHaveBeenCalledWith("Attempting to create Clean Energy");
      consoleInfoSpy.mockRestore();
    });
    ...
    it("does nothing when all sectors exist", async () => {
      const allExist = [
        ...mockWebflowSectors,
        { id: "webflow-4", fieldData: { name: "Healthcare", slug: "healthcare" } },
      ];
  ...
  describe("reSortSectors", () => {
    ...
    it("assigns ranks in correct order", async () => {
      ...
      expect(calls[1][2]).toEqual({ rank: 2 }); // Clean Energy (alphabetically first)
      expect(calls[2][2]).toEqual({ rank: 3 }); // Old Tech Name (alphabetically second)
    });
  });
