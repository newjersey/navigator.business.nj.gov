...
const mockedMatter = jest.mocked(require("gray-matter"));

const generateNavigatorFaq = (overrides?: Partial<Faq>): Faq => {
  return {
    name: `FAQ ${randomInt()}`,
    slug: `faq-${randomInt()}`,
...
describe("faqSync", () => {
  ...
  describe("createNewFaqs", () => {
    it("should create new FAQs in Webflow and update source markdown files with webflowId", async () => {
      ...
      mockedMatter.mockReturnValue({ data: { name: newFaq.name }, content: "content" });
