...
const getCertificationsOptions = async (): Promise<FundingCertificationOption[]> => {
  ...
  return itemResponse.map(({ id, fieldData }) => ({
    name: fieldData.name,
    slug: fieldData.slug,
    id,
  }));
...
const getFundingFromMd = (
  ...
  const industryReferenceArray = i.sector.map((sectorSlug) => {
    return sectors.find((v) => {
      return v.fieldData.slug === sectorSlug;
    })?.id;
  });
  ...
  return {
    ...
    "certifications-2": certifications.filter((c): c is string => c !== undefined),
    agency,
    ...
    "industry-reference":
      industryReferenceArray.length > 0
        ? industryReferenceArray.filter((id): id is string => id !== undefined)
        : [allIndustryId],
  };
};
...
const updateFundings = async (): Promise<void> => {
  ...
  const modify = async (item: WebflowItem<WebflowFundingFieldData>): Promise<void> => {
    const fundingData = fundings.find((i) => {
      return i.id === item.fieldData.slug;
    });
    if (!fundingData) return;

    const funding = getFundingFromMd(fundingData, sectors);
