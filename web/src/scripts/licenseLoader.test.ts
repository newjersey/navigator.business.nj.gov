...
Test content`;
  ...
  describe("loadNavigatorLicense", () => {
    ...
    it("throws error when file is not found in any directory", () => {
      ...
      expect(() => loadNavigatorLicense("nonexistent.md")).toThrow(
        "couldn't find file when trying to load from MD",
      );
    });
  });
  ...
  describe("writeMarkdownString", () => {
    beforeEach(() => {
      // Restore the real matter.stringify for these tests
      mockMatter.stringify = jest.requireActual("gray-matter").stringify;
    });
