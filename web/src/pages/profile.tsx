...
const ProfilePage = (props: Props): ReactElement => {
  ...
  const profileTabSlugs: Record<ProfileTabs, string> = useMemo(
    () =>
      ((
        Config.profileDefaults.default as typeof Config.profileDefaults.default & {
          profileTabSlugs?: Record<ProfileTabs, string>;
        }
      ).profileTabSlugs ?? {
  ...
  const getProfileTabFromQueryValue = useCallback(
    (tabValue: string | string[] | undefined): ProfileTabs | undefined => {
      ...
      return (
        (
          Object.fromEntries(
            Object.entries(profileTabSlugs).map(([key, value]) => [value, key as ProfileTabs]),
          ) as Record<string, ProfileTabs>
        )[tabValueAsString] ??
