...
export const fetchTaskLinkByFilename = async (
  filename: string
): Promise<TaskLink> => {
  const taskWithoutLinks = convertTaskMd(await fetchTaskFile(filename));
  return {
...
const fetchDependenciesFile = async (): Promise<TaskDependencies[]> => {
  const file = await (process.env.NODE_ENV === "test"
    ...
    : import(
        `@businessnjgovnavigator/content/roadmaps/task-dependencies.json`
      ));
...
const fetchTaskFile = async (filename: string): Promise<string> => {
  ...
  if (process.env.NODE_ENV === "test") {
    ...
    try {
      file = await import(
        `@businessnjgovnavigator/content/roadmaps/tasks/${filename}.md`
      );
    } catch {
      try {
        file = await import(
