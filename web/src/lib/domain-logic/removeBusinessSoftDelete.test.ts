...
describe("remove business soft delete", () => {
  it("sets deletion iso date", () => {
    ...
    expect(newUserData.businesses[secondBusiness.id].dateDeletedISO).not.toEqual(undefined);
    expect(newUserData.businesses[firstBusiness.id].dateDeletedISO).toEqual(undefined);
  });
});
