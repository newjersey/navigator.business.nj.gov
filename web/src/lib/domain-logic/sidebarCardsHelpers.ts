...
export default class SidebarCardsHelpers {
  getVisibleCertifications(certifications: Certification[], business: Business | undefined): Certification[] {
    if (!business) return [];
  ...
  getHiddenCertifications(business: Business | undefined, certifications: Certification[]): Certification[] {
    if (!business) return [];
  ...
  sortCertifications(certifications: Certification[]): Certification[] {
  ...
  getForYouCardCount(
    ...
    if (LookupOperatingPhaseById(business?.profileData.operatingPhase).displayCertifications) {
      count += this.getVisibleCertifications(
        this.filterCertifications({ certifications, business }),
        business
      ).length;
    }

    if (LookupOperatingPhaseById(business?.profileData.operatingPhase).displayFundings) {
      count += this.getVisibleFundings(this.filterFundings({ fundings, business }), business).length;
    }
  ...
  sortFundings(fundings: Funding[]): Funding[] {
  ...
  getVisibleFundings(fundings: Funding[], business: Business | undefined): Funding[] {
    if (!business) return [];
    return fundings.filter((it) => {
      return !business?.preferences.hiddenFundingIds.includes(it.id);
    });
  }
  getHiddenFundings(business: Business | undefined, fundings: Funding[]): Funding[] {
    if (!business) return [];
    return fundings.filter((it: Funding) => {
      return business.preferences.hiddenFundingIds.includes(it.id);
    });
  }
  filterFundings({ fundings, business }: { fundings?: Funding[]; business?: Business }): Funding[] {
